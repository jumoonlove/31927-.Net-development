...
const setGameBackground = () => {
    let parallaxValue = Math.min(0, (hero.moveSpeed - gameProp.screenWidth/3) * -1);

    gameBackground.gameBox.style.transform = `translateX(${parallaxValue}px)`;
}

const setHeroMovement = () => {
...
const loadImg = () => {
    const preLoadImgSrc = ['/Javascript_game/lib/images/ninja_attack.png', 
    '/Javascript_game/lib/images/ninja_run.png', 
    '/Javascript_game/lib/images/ninja_slide.png'];
...
const init = () => {
    hero = new Hero('.hero'); // create hero instance
    stageInfo.stage = new Stage();
    npcOne = new Npc(levelQuest);
    npcTwo = new Npc(levelQuestTwo);
    loadImg(); // load attack and run images before execute CSS classes to prevent blinking
    windowEvent();
    renderGame();
}
