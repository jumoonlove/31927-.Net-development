class Npc {
    ...
    position(){ // to find position of hero
        return{
...
class Stage {
    ...
    callMonster(){
        for (let i=0; i <= 10; i++){
            if(i === 10){
                allMonsterComProp.arr[i] = new Monster(stageInfo.monster[this.level].bossMon, hero.moveSpeed + gameProp.screenWidth + 600 * i);
            }
            else
            allMonsterComProp.arr[i] = new Monster(stageInfo.monster[this.level].defaultMon, gameProp.screenWidth + hero.moveSpeed + 700 * i);
        }
    }
    clearCheck(){
        stageInfo.callPosition.forEach( arr => {
            if(hero.moveSpeed >= arr && allMonsterComProp.arr.length === 0){
                this.stageGuide('Monsters Coming !!')
...
class Hero {
    ...
    keyMotion(){ // changes the motion of hero when key pressed
        if(key.keyDown['left']){
            this.direction = 'left';
            this.el.classList.add('run');
            this.el.classList.add('flip')

            this.moveSpeed = this.moveSpeed <= 0 ? 0 : this.moveSpeed - this.speed;
        }
        ...
        if(key.keyDown['slide']){
            ...
            if(!this.slideDown){
                ...
                if(this.direction === 'right'){
                    this.moveSpeed = this.moveSpeed + this.slideSpeed;
                }
                else{
                    this.moveSpeed = this.moveSpeed <= 0 ? 0 : this.moveSpeed - this.slideSpeed;
                }
        ...
        this.el.parentNode.style.transform = `translateX(${this.moveSpeed}px)`;
    }
...
class Bullet{
    ...
    init(){
        ...
        this.x = this.bulletDirection === 'right' ? hero.moveSpeed + hero.size().width / 2 : hero.moveSpeed - hero.size().width / 2;
    ...
    position(){ // to find position of bullet
        return{
            left: this.el.getBoundingClientRect().left,
            right: this.el.getBoundingClientRect().right,
            top: gameProp.screenHeight - this.el.getBoundingClientRect().top,
...
class Monster {
    constructor(property, positionX){
        ...
        this.defaultHpVale = property.hpValue;
    ...
    updateHp(index){
        ...
        this.progress = this.hpValue / this.defaultHpVale * 100;
    ...
    moveMonster(){
        if(this.moveSpeed + this.positionX + this.el.offsetWidth + hero.position().left - hero.moveSpeed<= 0){
            this.moveSpeed = hero.moveSpeed - this.positionX + gameProp.screenWidth - hero.position().left;
        }
        else{
            this.moveSpeed -= this.speed;
        }
        this.el.style.transform = `translateX(${this.moveSpeed}px)`;
        this.crash();
    }
    crash(){
        let rightDiff = 30;
